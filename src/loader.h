...
public:
	static Mesh *load_obj(std::string inputfile) {
		...
		for (size_t shape_index = 0; shape_index < shapes.size();
			...
			for (size_t face_index = 0;
				 ...
				 face_index++) {
				size_t current_face_vertices_num = size_t(
	...
	static std::vector<Hittable *> load_cornell_box() {
		...
		glm::mat4 obj_translation =
			glm::translate(glm::mat4(1.0f), glm::vec3(150, 300, 450));
		glm::mat4 obj_transform =
			glm::scale(obj_translation, glm::vec3(100, 100, 100));
