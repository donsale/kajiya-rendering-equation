...
kajiya::Color constrain_rgb(double r, double g, double b) {
	double w;
...
/*CIE 2018, CIE 1931 colour-matching functions , 2 degree observer (data table),
 * International Commission on Illumination (CIE), Vienna, Austria,
 * DOI:10.25039/CIE.DS.xvudnb9b*/
float cmf[471][3] = {{0.0001299, 0.000003917, 0.0006061},
...
kajiya::Color xyz_to_rgb(double xc, double yc, double zc) {
	...
	kajiya::Color rgb = constrain_rgb((rx * xc) + (ry * yc) + (rz * zc),
									  (gx * xc) + (gy * yc) + (gz * zc),
									  (bx * xc) + (by * yc) + (bz * zc));
...
kajiya::Spectrum luminous_intensity() {
	...
	for (int i = 420; i <= 475; i++) {
		emittance.spectrum[i - 360] = lerp(0, 0.12, (i - 420.f) / (55.f));
	}

	for (int i = 475; i <= 560; i++) {
		emittance.spectrum[i - 360] = lerp(0.12, 1, (i - 475.f) / (85.f));
	}

	for (int i = 560; i <= 650; i++) {
		emittance.spectrum[i - 360] = lerp(1, 0.12, (i - 560.f) / 90.f);
	}

	for (int i = 650; i <= 700; i++) {
		emittance.spectrum[i - 360] = lerp(0.12, 0, (i - 650.f) / 50.f);
	}
