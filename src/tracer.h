...
kajiya::Spectrum Lr(kajiya::Hittable *object, kajiya::IntersectionData &intersection_data, kajiya::Ray &r,
...
kajiya::Spectrum Li(kajiya::Ray &ray, std::vector<kajiya::Hittable *> objects,
	...
	if (intersection_data_opt.has_value()) {
		...
		kajiya::Spectrum spectrum = closest->material(intersection_data_opt.value()).emittance;

		if (depth > 0)
			spectrum = spectrum + Lr(closest, intersection_data_opt.value(), new_ray, objects, depth - 1);
...
kajiya::Spectrum Lr(kajiya::Hittable *object, kajiya::IntersectionData &intersection_data, kajiya::Ray &r, std::vector<kajiya::Hittable *> objects, int depth) {
	...
	if (object->material(intersection_data).type != kajiya::Material::light) {
		...
		if (intersection_data_opt_temp.has_value()) {
			if (closest_temp->material(intersection_data).type == kajiya::Material::light) {
				float light_dist_squared =
					(intersection_data_opt_temp.value().point - r.origin).norm_squared();
				...
				float light_dot = kajiya::Vec3::dot(
					...
					(intersection_data_opt_temp.value().point - r.origin).unit());
	...
	if (object->material(intersection_data).type == kajiya::Material::metal) {
		...
		relevant_object_spectrum = object->material(intersection_data).reflectance;
	} else if (object->material(intersection_data).type == kajiya::Material::glass) {
		kajiya::Vec3 surface_normal = object->normal(intersection_data);
		float refractive_index_1	= r.refractive_index;
		float refractive_index_2	= object->material(intersection_data).refractive_index;
		...
		if (rand_float() < fresnel_reflection_coef) {
			...
			relevant_object_spectrum = object->material(intersection_data).transmittance;
		}
	} else {
		new_direction =
	...
	if (intersection_data_opt.has_value()) {
		if (closest->material(intersection_data_opt.value()).type != kajiya::Material::light) {
